...
\begin{minipage}{0.48\textwidth}
    ...
    \node[left] at (0,{3/sqrt(1)}) {$\mu = G_\text{max}$};
    \draw[dashed] (0,{3/(sqrt(1)*sqrt(2))}) node[left] {$\frac{G_\text{max}}{\sqrt{2}}$} -- (1,{3/(sqrt(1)*sqrt(2))});
    \draw[dashed] (1,0) node[below] {$\bar \omega$} -- (1,{3/(sqrt(1)*sqrt(2))});
	\end{tikzpicture}
\end{minipage}
...
Il sistema si dice \textbf{filtro passa basso}, mentre chiamamo $\bar \omega $ \textbf{frequenza di taglio}. Definiamo inoltre
\begin{equation*}
