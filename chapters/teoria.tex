...
\begin{minipage}{0.48\textwidth}
    ...
    \draw[dashed] (0,3) node[left] {$\mu = G_\text{max}$} -- (25,3);
    \draw[dashed] (0,{3/sqrt(2)}) node[left] {$\frac{G_\text{max}}{\sqrt{2}}$} -- (10,{3/sqrt(2)});
    \draw[dashed] (1,0) node[below] {$\bar \omega$} -- (1,{3/sqrt(2)});
    \draw[dashed] (10,0) node[below] {$\bar {\bar \omega}$} -- (10,{3/sqrt(2)});
    \draw[<->, >=stealth] (1,{3/(2*sqrt(2))}) -- (10,{3/(2*sqrt(2))}) node[midway, above] {$BP$};
